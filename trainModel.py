...
DATASET_PATH = '/s/chopin/l/grad/eseefrie/wildfires/data/next-day-wildfire-spread'
SAVE_MODEL_PATH = '/s/babbage/b/nobackup/nblancha/eseefrie/cs535-term-project/savedModels'


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-m','--master', default='earth',
                        help='master node')
    parser.add_argument('-p', '--port', default='30138',
                         help = 'master node')
...
def create_data_loaders(rank, gpu, world_size):
    ...
    datasets = {
        TRAIN: RotatedWildfireDataset(
            ...
            features= [0,3,4,5,6,7,8,9,10,11]
        ),
        VAL: WildfireDataset(
...
def perform_validation(model, loader):
    ...
    for i, (images, labels) in enumerate(loader):
        ...
        loss = torchvision.ops.sigmoid_focal_loss(outputs, labels, alpha=0.9, gamma=1, reduction="mean")
...
def train(gpu, args):
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr=0.0001, momentum=0.9)
